...
class DbService {
  ...
  static Future<String> initDb() async {
    ...
    if (!isDbExists) {
      ...
      ByteData data = await rootBundle.load(AppConstants.dbSchemaFile);
      List<int> bytes =
  ...
  static Future<Map<int, double>> getDayOfWeekSummary() async {
    ...
    final date = DateTime(now.year, now.month, now.day)
        .subtract(const Duration(days: 30));
    final Map<int, double> dowMap = {
      for (var item in Iterable.generate(7, (index) => index + 1)) item: 0.0
    };
    final List<Map<String, Object?>> result = await db.rawQuery(
      '''
      SELECT 
  ...
  // static Future<List<Attribute>> getGoodMoodRelatedAttributeByCategory(
  //     AttributeCategory category) async {
  ...
  // static Future<List<Attribute>> getBadMoodRelatedAttributeByCategory(
  //     AttributeCategory category) async {
  ...
  // static Future<List<Attribute>> getAttributesByCategory(
  //     AttributeCategory category) async {
  //   final db = await getDb();
  //   final List<Map<String, Object?>> result = await db.rawQuery('''
  //     SELECT a.*
  ...
  //   await db.rawDelete(
  //       'DELETE FROM sqlite_sequence WHERE name = ?', [Mood.dbTable]);
  // }
}
